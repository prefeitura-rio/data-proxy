...
| `n`         | no       | Keep only the last `n` partitions. Applies to time-partitioned tables only. Older partitions remain available in BigQuery for an enabled fallback.                                                                                                                                                                                                                                               |
| `fallback`  | no       | Set to `false` to disable the `_bq` fallback view for this table. Defaults to `true` when chart fallback is enabled.                                                                                                                                                                                                                                                                             |
| `cache_ttl` | no       | Cache lifetime in seconds for this table. Overrides `fallback.cacheTtl`.                                                                                                                                                                                                                                                                                                                         |
...
When chart fallback is enabled, the Publisher also creates a `<table>_bq` view for each table with fallback enabled. The view uses the same RLS policy as the local table. The proxy uses this view only after an empty local read. See [BigQuery Fallback](fallback.md).
...
BigQuery `RECORD` and `STRUCT` columns are automatically converted to JSON during extraction. Data Proxy loads these columns as PostgreSQL `json`, then alters them to `jsonb` after loading. This two-step approach is required because pg_duckdb can not write directly into `jsonb` columns through `read_parquet`. The ALTER happens inside a transaction after the data is loaded, enabling GIN index support for efficient JSON path queries through PostgREST.
