...
let config = try { open /config/oidc.nuon } catch {|err|
    log error $'Failed to open OIDC config: ($err.msg)'
    exit 1
}
...
def decode-jwt-payload [token: string]: nothing -> record {
    $token | parse '{header}.{payload}.{sig}' | get payload | b64url-decode $in | try { from json } catch { {} }
}
