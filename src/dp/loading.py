...
from .models import PublicationResult, SyncConfig, SyncPlan
...
def apply_sync_plan(
    ...
    empty_incremental = {
        table
        for table, table_plan in publication_plan.partitioned_tables.items()
    ...
    for table_name in decision.blocked_tables:
        record_table_failure(pg_conn, tables[table_name], plan, attempted_at)
    ...
    prepared = prepare_tables(pg_conn, duckdb_conn, config, publication_plan, eligible)
    logger.info("Prepared {} tables", len(prepared))

    prepared_names = {table.name for table in prepared}
