...
- Keep changes small and reviewable. Don't bundle unrelated work.
...
- Use `asyncer.asyncify` for blocking calls. Don't write hand-rolled thread helpers.
- Prefer explicit parameters and named functions over dynamic `**kwargs` and opaque lambdas.
- Group related functions by domain.
- Use generic names in reusable models. Don't encode one customer or table in a generic field.
- Treat JWT claims as identity. Read authorization state from the database.
- Don't make outbound HTTP calls from request-time authorization logic.

## Nushell

Nushell scripts power Helm maintenance jobs and local development commands. Keep them small and typed.

- Declare input and output types on custom commands. Don't use `any` when a concrete type is available.
...
- Handle external commands at the command boundary. Log failures and re-raise them when the caller can't continue.
- Use `use std/log` and `log` for operational output. Don't use `print` for service output.
- Use `use ./lib.nu [quote-pg render-sql]` for SQL rendering.
- Use `quote-pg` for values that reach raw SQL. Don't interpolate unquoted values into `psql -c`.
...
- Use `SECURITY DEFINER` only for deliberate privileged database functions. Grant run to the required role only.
...
- Use set-based loads and deletes. Don't insert or delete rows one at a time.

### Backend rules

- PostgreSQL uses `pg_duckdb` to read Parquet from object storage. Don't download Parquet to local disk first.
- DuckDB runs in a worker thread through `asyncer.asyncify`. Don't block the event loop.
...
- Use a service account or Workload Identity. Don't embed credentials in an image.

## Configuration and state

- Keep one source of truth for each fact. Don't synchronize duplicate configuration.
...
- Don't re-implement DBOS checkpointing in application code.
...
- Verify runtime claims against source, Helm templates, tests, or an ran command.

## Git

- Follow Conventional Commits: `type(scope)!: imperative summary`. Use lowercase and no trailing interval.
...
- Use `!` for a compatibility-breaking change. Explain the migration in the commit body. Don't use a `BREAKING CHANGE` footer.
...
- Pure documentation clarifications, typo fixes, and single-file wording corrections don't need a plan.
