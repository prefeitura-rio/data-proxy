...
function buildMetrics(token: string): MetricRequest[] {
  ...
  metrics.push(
    redisMetric(
      "db_size",
      "the pipeline database is small",
      "DBSIZE",
      token,
      (r) => {
        if (r.status !== 200) return -1;
...
function pollOnce(metrics: MetricRequest[]): boolean {
  ...
  metrics.forEach((m, i) => {
    ...
    if (
      m.metric.startsWith("stream_length:") &&
      typeof value === "number" &&
      value !== 0
    ) {
          ...
          containers: [
            {
              ...
              args: [
                "-c",
                `psql "$PG_DSN" -v ON_ERROR_STOP=1 -c ${JSON.stringify(statement)}`,
              ],
...
function verifyIncrementalRestore(
  ...
  requirePrecondition(
    "the partitioned table has a stored manifest",
    state !== null && state.partitions !== null,
    { state: state },
  );

  const partitions = (state as StoredState).partitions as Record<
    string,
    StoredPartition
  >;
  ...
  requirePrecondition(
    "the changed partition has one freshness row",
    before.length === 1,
    { rows: before.length },
  );
  ...
  const reduced: StoredState = {
    ...(state as StoredState),
    partitions: { ...partitions },
  };
  delete (reduced.partitions as Record<string, StoredPartition>)[target];
  requirePrecondition(
    "the partition is removed from the stored manifest",
    writeState(token, PARTITIONED_SOURCE, reduced),
    {},
  );
  ...
  expect(
    "the stored manifest holds the partition again",
    readState(token, PARTITIONED_SOURCE)?.partitions?.[target] !== undefined,
  );
}
...
function verifyShadowRebuild(
  ...
  requirePrecondition(
    "the full table state key is deleted",
    deleteKey(token, `dp:state:${FULL_SOURCE}`),
    {},
  );
